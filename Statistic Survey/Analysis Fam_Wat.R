...
# Analysis in Between Family Expenditure vs Per Water Consumption
library(ggplot2)
...
pop_size = 18055
sam_size = length(Dataset$FAM_EXP_SUM)

#calculate sample means
y_mean = mean(Dataset$PER_WAT_CON)
x_mean = mean(Dataset$FAM_EXP_SUM)
...
yx_sum = sum(Dataset$PER_WAT_CON * Dataset$FAM_EXP_SUM) 
...
ggplot(confidence_data, aes(x = 1, y = Estimate)) + 
  ...
  labs(title = "Confidence Interval for the Ratio Per Water Count VS Family Expenditure",
       x = "",
       y = "Ratio Estimate") +
  theme_minimal() +
...
#
correlation <- cor(Dataset$PER_WAT_CON, Dataset$FAM_EXP_SUM)
print(paste("Correlation between Per Water Count and Family Expenditure:", correlation))

# Plotting the relationship with a linear regression line
ggplot(Dataset, aes(x = FAM_EXP_SUM, y = PER_WAT_CON)) + 
  ...
  labs(title = "Relationship between between Per Water Count VS Family Expenditure",
       x = "Family Expenditure",
       y = "Family Water Consumption Per Capita") +
  theme_minimal()

# Optionally, save the plot
ggsave("correlation_plot_Fam_Wat.png", width = 8, height = 6)
